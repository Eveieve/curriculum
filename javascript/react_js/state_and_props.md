...
`App` component imports `MyComponent` component, and renders `MyComponent` as its child. In `App`'s  JSX, we pass down a property called `title`. The syntax should look familiar to you - it's the same as how we assign attributes to to HTML elements. In React, we call `title` a 'prop' (short for *property*, as in object property). Next, we're setting the value "React" to the `title` prop. In `MyComponent` , we read the prop `title`'s value "React" with `this.props.title`. This technique is called "passing props". 

*IMPORTANT*: Make sure you pass `props`  to both the `constructor()` and the `super()` method of the child component (`MyComponent`). You won't be able to access `this.props.title` in `MyComponent` otherwise. If you're unfamiliar with what `super` does, checkout the docs. For now, it's enough to understand that you **must** pass `props`  to both the `constructor()` and `super()`. 
